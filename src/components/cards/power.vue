<template>
    <v-row density="comfortable" v-if="powerSensors.length > 0">
        <v-col cols="12">
    ...
    <div v-else class="text-body-2 text-medium-emphasis pa-4 text-center">
        <div>{{ $t('no power data available') }}</div>
        <div>{{ $t('please add power sensors in mos settings') }}</div>
    </div>
</template>
...
<script setup>
    ...
    const powerSensors = computed(() => sensors.value?.power ?? [])

    const sortedSensors = computed(() => {
        return [...powerSensors.value].sort((a, b) => {
