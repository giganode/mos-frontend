<template>
    <v-row density="comfortable" v-if="temperatureSensors.length > 0">
        <v-col cols="12">
...
<script setup>
    ...
    const temperatureSensors = computed(() => sensors.value?.temperature ?? [])

    const sortedSensors = computed(() => {
        return [...temperatureSensors.value].sort((a, b) => {
