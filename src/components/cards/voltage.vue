<template>
    <v-row density="comfortable" v-if="voltageSensors.length > 0">
        <v-col cols="12">
    ...
    <div v-else class="text-body-2 text-medium-emphasis pa-4 text-center">
        <div>{{ $t('no voltage data available') }}</div>
        <div>{{ $t('please add voltage sensors in mos settings') }}</div>
    </div>
</template>
...
<script setup>
    ...
    const voltageSensors = computed(() => sensors.value?.voltage ?? [])

    const sortedSensors = computed(() => {
        return [...voltageSensors.value].sort((a, b) => {
