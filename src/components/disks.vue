<template>
  <div v-if="props.pools && props.pools.length" style="overflow: hidden">
    <div v-for="(pool, poolIdx) in props.pools" :key="poolIdx" class="mb-1" style="overflow: hidden">
      <div class="d-flex align-center justify-space-between mb-0 mt-2" style="overflow: hidden">
        <div class="text-subtitle-2 font-weight-medium" style="min-width: 0; line-height: 1.0; overflow: hidden">
        ...
        <div class="text-caption text-medium-emphasis" style="line-height: 1.0; overflow: hidden">{{ (pool.data_devices?.length ?? 0) + (pool.parity_devices?.length ?? 0) }} {{ $t('device(s)') }}</div>
      </div>
      ...
      <template v-if="(pool.data_devices && pool.data_devices.length) || (pool.parity_devices && pool.parity_devices.length)">
        <v-row v-for="(data_device, devIdx) in pool.data_devices ?? []" :key="`data-${devIdx}`" dense class="py-0" style="flex-wrap: nowrap; overflow: hidden">
          ...
          <v-col cols="2" sm="2" class="py-1" style="overflow: hidden">
            <div class="text-body-2" style="font-variant-numeric: tabular-nums; line-height: 1.1; overflow: hidden">
        ...
        <v-row v-for="(parity_device, devIdx) in pool.parity_devices ?? []" :key="`parity-${devIdx}`" dense class="py-0" style="flex-wrap: nowrap; overflow: hidden">
          <v-col cols="4" sm="4" class="d-flex align-center py-1" style="gap: 6px; min-width: 0; overflow: hidden">
            ...
            <div class="text-body-2" :title="parity_device.device" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; line-height: 1.1">
              ...
              <span class="text-caption text-medium-emphasis" style="line-height: 1.1; font-size: inherit; display: inline-block; vertical-align: baseline; margin-left: 6px; overflow: hidden">(parity)</span>
            </div>
          </v-col>
          <v-col cols="2" sm="2" class="py-1" style="overflow: hidden">
            <div class="text-body-2" style="font-variant-numeric: tabular-nums; line-height: 1.1; overflow: hidden">
