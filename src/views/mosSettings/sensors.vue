                                        ...
                                        <tr v-for="s in section.items" :key="s.id ?? `${section.key}-${s.index}-${s.name}`">
                                            ...
                                            <td>
                                                <div class="text-body-2"
                                                     style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 520px">
                                                    {{ s.name }}
                                                </div>
                                            </td>
                                            <td><span class="text-body-2">{{ s.subtype || '-' }}</span></td>
                                            <td><span class="text-body-2">{{ s.manufacturer || '-' }}</span></td>
                                            <td><span class="text-body-2">{{ s.model || '-' }}</span></td>
                                            <td class="text-right">
                                                <span class="text-body-2">
                                                    {{ (s.value === '' || s.value === null || s.value === undefined) ? '-' : formatValue(s.value) }}
                                                </span>
                                            </td>
                                            <td><span class="text-body-2">{{ s.unit }}</span></td>
                                            <td>
        ...
        <v-card class="pa-0">
            ...
            <v-card-text>
                <v-form>
                    ...
                    <v-select v-model="createSensorDialog.type"
                              :items="sensorTypes"
                              :label="$t('type')"
                              dense />
                    <v-select v-model="createSensorDialog.subtype"
                              :items="sensorSubtypes"
                              :label="$t('subtype')"
                              dense
                              clearable />
                    ...
                    <v-text-field v-model="createSensorDialog.unit"
                                  :label="$t('unit')"
                                  clearable
                                  class="mt-4" />
                    <v-text-field v-model="createSensorDialog.manufacturer"
                                  :label="$t('manufacturer')"
                                  clearable />
                    <v-text-field v-model="createSensorDialog.model"
                                  :label="$t('model')"
                                  clearable />
                    <v-divider class="my-2" />
                    <v-text-field v-model="createSensorDialog.value_range"
                                  :label="$t('value range')"
                                  placeholder="null"
                                  clearable />
                    <v-select v-model="createSensorDialog.transform"
                              :items="sensorTransformations"
                              :label="$t('transform')"
                              dense
                              clearable />
                </v-form>
            ...
            <v-card-actions>
                <v-btn @click="createSensorDialog.value = false"
                       color="onPrimary">{{ $t('cancel') }}</v-btn>
                <v-btn @click="createSensor()"
                       color="onPrimary">{{ $t('create') }}</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
        ...
        <v-card class="pa-0">
            ...
            <v-card-text>
                <v-form>
                    ...
                    <v-select v-model="editSensorDialog.type"
                              :items="sensorTypes"
                              :label="$t('type')"
                              dense />
                    <v-select v-model="editSensorDialog.subtype"
                              :items="sensorSubtypes"
                              :label="$t('subtype')"
                              dense
                              clearable />
                    <v-autocomplete v-model="editSensorDialog.source"
                                    ...
                                    clearable
                                    :loading="unmappedLoading" />
                    <v-text-field v-model="editSensorDialog.unit"
                                  :label="$t('unit')"
                                  clearable
                                  class="mt-4" />
                    <v-text-field v-model="editSensorDialog.manufacturer" 
                                  :label="$t('manufacturer')"
                                  clearable />
                    <v-text-field v-model="editSensorDialog.model" 
                                  :label="$t('model')"
                                  clearable />
                    <v-divider class="my-2" />
                    <v-text-field v-model="editSensorDialog.value_range"
                                  :label="$t('value range')"
                                  placeholder="null"
                                  clearable />
                    <v-select v-model="editSensorDialog.transform"
                              :items="sensorTransformations"
                              :label="$t('transform')"
                              dense
                              clearable />
            ...
            <v-card-actions>
                <v-btn @click="editSensorDialog.value = false"
                       color="onPrimary">{{ $t('cancel') }}</v-btn>
                <v-btn @click="updateSensor()"
                       color="onPrimary">{{ $t('save') }}</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
  ...
  for (const [chip, chipObj] of Object.entries(root)) {
    ...
    for (const [group, groupObj] of Object.entries(chipObj)) {
      ...
      for (const [key, val] of Object.entries(groupObj)) {
        ...
        const valTxt =
          typeof val === 'number' ? ` = ${Number(val).toFixed(2).replace(/\.00$/, '')}` : ''
