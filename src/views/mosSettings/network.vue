                    ...
                    <v-card-text>
                        <v-form>
                            ...
                            <v-row v-for="(iface, idx) in settingsNetwork.interfaces" :key="idx">
                                ...
                                <v-col  v-if="iface.ipv4 && iface.ipv4.length" cols="12" class="pt-0 pb-0">
                                    <v-switch :label="$t('ipv4 dhcp')" v-model="iface.ipv4[0].dhcp" inset
                                        density="compact" color="primary"></v-switch>
                                </v-col>
                                <v-col v-if="iface.ipv4 && iface.ipv4.length" cols="12" class="pt-0 pb-0">
                                    <v-text-field :label="$t('ipv4 address')"
                                        v-model="iface.ipv4[0].address"></v-text-field>
                                </v-col>
                                <v-col v-if="iface.ipv4 && iface.ipv4.length" cols="12" class="pt-0 pb-0">
                                    <v-text-field :label="$t('ipv4 gateway')"
                                        v-model="iface.ipv4[0].gateway"></v-text-field>
                                </v-col>
                                <v-col v-if="iface.ipv4 && iface.ipv4.length" cols="12" class="pt-0 pb-0">
                                    <v-text-field
                                ...
                                <v-col v-if="iface.ipv6 && iface.ipv6.length" cols="12" class="pt-0 pb-0">
                                    <v-switch :label="$t('ipv6 dhcp')" v-model="iface.ipv6[0].dhcp" inset
                                        density="compact" color="primary"></v-switch>
                                </v-col>
                                <v-col v-if="iface.ipv6 && iface.ipv6.length" cols="12" class="pt-0 pb-0">
                                    <v-text-field :label="$t('ipv6 address')"
                                        v-model="iface.ipv6[0].address"></v-text-field>
                                </v-col>
                                <v-col v-if="iface.ipv6 && iface.ipv6.length" cols="12" class="pt-0 pb-0">
                                    <v-text-field :label="$t('ipv6 gateway')"
                                        v-model="iface.ipv6[0].gateway"></v-text-field>
                                </v-col>
                                <v-col v-if="iface.ipv6 && iface.ipv6.length" cols="12" class="pt-0 pb-0">
                                    <v-text-field
                            ...
                            <v-text-field :label="$t('netbird service params')"
                                v-model="settingsNetwork.services.netbird.netbird_service_params"></v-text-field>
                        </v-form>
                    </v-card-text>
                </v-card>
            </v-container>
...
const getNetworkSettings = async () => {
    try {
        ...
        if (!res.ok) throw new Error(t('network settings could not be loaded'));
...
const setNetworkSettings = async () => {
    try {
        ...
        if (!res.ok) throw new Error(t('network settings could not be changed'));
