        ...
        <v-card fluid style="margin-bottom: 80px" class="pa-0">
          <v-card-text class="pa-0">
            ...
            <v-container fluid class="pa-4">
              <v-row>
                <v-col v-if="onlineTemplates.length > 0" cols="12" sm="6" md="4" lg="3" v-for="(tpl, i) in onlineTemplates" :key="tpl.name || i">
                  <v-card style="height: 250px; display: flex; flex-direction: column" class="pa-0">
                    ...
                    <v-card-title>
                      <div class="text-h6 font-weight-bold" style="line-height: 1.3">{{ tpl.name || $t('unknown') }}</div>
                    </v-card-title>
                    <v-card-text style="flex: 1 1 auto; overflow: hidden; padding-top: 0; margin-bottom: 10px">
                      <div v-if="tpl.description" class="text-body-2" style="display: -webkit-box; -webkit-line-clamp: 4; -webkit-box-orient: vertical; overflow: hidden">
                        {{ tpl.description }}
                      </div>
                      <div v-else class="grey--text text--caption font-italic">{{ $t('no description') }}</div>
                    </v-card-text>
                    <v-divider />
                    <v-card-actions style="flex: 0 0 auto; gap: 8px; padding: 12px">
                      <v-btn
                        variant="tonal"
                        color="primary"
                        :href="tpl.link || tpl.repository || tpl.documentation"
                        target="_blank"
                        v-if="tpl.link || tpl.repository || tpl.documentation"
                        prepend-icon="mdi-eye"
                        size="small"
                      >
                        {{ $t('view') }}
                      </v-btn>
                      <v-btn variant="outlined" color="secondary" :href="tpl.url" target="_blank" v-if="tpl.url" prepend-icon="mdi-web" size="small">
                        {{ $t('webpage') }}
                      </v-btn>
                    </v-card-actions>
                  </v-card>
                ...
                <v-col v-else cols="12">
                  <div class="text-center grey--text text--darken-1">{{ $t('no templates found matching your search') }}</div>
                </v-col>
              </v-row>
            </v-container>
          </v-card-text>
